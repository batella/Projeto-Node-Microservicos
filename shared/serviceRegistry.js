...
console.log('File-based Service Registry inicializado:',
this.registryFile);
}
ensureRegistryFile() {
...
console.log(`Serviço registrado: ${serviceName} - ${serviceInfo.url} (PID:
${process.pid})`);
...
const services = this.readRegistry();
console.log(`Executando health checks de ${Object.keys(services).length}
serviços...`);
...
console.error(`Health check falhou para ${serviceName}:`,
error.message);
...
'healthy' : 'unhealthy'}) PID:${service.pid}`);
});
}
