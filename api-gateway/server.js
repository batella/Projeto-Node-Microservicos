...
console.error(` Erro na descoberta do serviço ${serviceName}: ❌`,
error.message);
...
console.log(`🔗 Encaminhando erro ${error.response.status} do
serviço`);
...
const [userResponse, productsResponse, categoriesResponse] = await
Promise.allSettled([
this.callService('user-service', '/users', 'GET', authHeader,
{ limit: 5 }),
this.callService('product-service', '/products', 'GET', null,
{ limit: 5 }),
...
data: userResponse.status === 'fulfilled' ?
userResponse.value.data : null,
error: userResponse.status === 'rejected' ?
userResponse.reason.message : null
},
products: {
available: productsResponse.status === 'fulfilled',
data: productsResponse.status === 'fulfilled' ?
productsResponse.value.data : null,
error: productsResponse.status === 'rejected' ?
productsResponse.reason.message : null
},
categories: {
available: categoriesResponse.status === 'fulfilled',
data: categoriesResponse.status === 'fulfilled' ?
categoriesResponse.value.data : null,
error: categoriesResponse.status === 'rejected' ?
categoriesResponse.reason.message : null
}
}
};
res.json({
...
if (authHeader) {
searches.push(
this.callService('user-service', '/search', 'GET', authHeader,
{ q, limit: 5 })
);
}
const [productResults, userResults] = await
Promise.allSettled(searches);
const results = {
query: q,
products: {
available: productResults.status === 'fulfilled',
results: productResults.status === 'fulfilled' ?
productResults.value.data.results : [],
error: productResults.status === 'rejected' ?
productResults.reason.message : null
}
};
...
results: userResults.status === 'fulfilled' ?
userResults.value.data.results : [],
error: userResults.status === 'rejected' ?
userResults.reason.message : null
};
}
res.json({
success: true,
